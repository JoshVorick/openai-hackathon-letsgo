...
async function updateHistoricalReservations() {
  ...
  try {
    ...
    for (const [dayStr, dayRates] of ratesByDay) {
      ...
      const reservationRate = isWeekendDay ? 0.90 : 0.60;
      ...
      if (ratesByDay.size <= 10 || totalUpdated % 1000 === 0) {
        console.log(`✓ ${dayStr} (${isWeekendDay ? 'Weekend' : 'Weekday'}): ${numToReserve}/${dayRates.length} rooms reserved (${Math.round(reservationRate * 100)}%)`);
      }
    }
