...
const WEEKEND_REGEX = /weekend|saturday|sunday/i;
const DATE_CONTEXT_REGEX = /this\s+(week|month|weekend)/i;
const YEAR_REGEX = /20\d{2}/;
const EVENT_TYPE_REGEX = /(hackathon|conference|festival|event)/i;
const PROMO_REGEX = /promo|discount|offer/i;
...
const recognitionPatterns: RecognitionPattern[] = [
  // Halloween and special events pricing
  {
    ...
    actionType: "pricing",
    message:
      "I can update your Halloween weekend pricing! Want me to analyze demand and optimize rates?",
    confidence: 0.95,
    extractParameters: (text) => {
      const percentMatch = text.match(PERCENT_REGEX);
      const isHalloween = text.toLowerCase().includes("halloween");
      return {
        percentage: percentMatch ? Number.parseInt(percentMatch[1], 10) : null,
  ...
  // General pricing adjustments
  {
    ...
    actionType: "pricing",
    message:
      "I can help adjust your hotel pricing! Want me to analyze the impact and implement rate changes?",
    confidence: 0.9,
    extractParameters: (text) => {
      const percentMatch = text.match(PERCENT_REGEX);
      const monthMatch = text.match(MONTHS_REGEX);
      return {
        percentage: percentMatch ? Number.parseInt(percentMatch[1], 10) : null,
        month: monthMatch ? monthMatch[1].toLowerCase() : null,
        reason: text.toLowerCase().includes("construction")
          ? "construction"
          : null,
      };
    },
  ...
  // Rate increases
  {
    ...
    actionType: "pricing",
    message:
      "I can help increase your rates! Want me to analyze demand patterns and suggest optimal pricing?",
    confidence: 0.85,
    extractParameters: (text) => {
      const percentMatch = text.match(PERCENT_REGEX);
  ...
  // Occupancy analysis
  {
    ...
    message:
      "I can analyze your occupancy data and booking patterns! Want me to generate insights?",
    confidence: 0.8,
    extractParameters: (text) => {
      const weekendMatch = text.match(WEEKEND_REGEX);
      const dateMatch = text.match(DATE_CONTEXT_REGEX);
      return {
        timeframe: weekendMatch
          ? "weekend"
          : dateMatch
            ? dateMatch[1]
            : "current",
  ...
  // Revenue analysis
  {
    ...
    actionType: "revenue",
    message:
      "I can generate revenue reports and year-over-year comparisons! Want me to create charts and insights?",
    confidence: 0.85,
    extractParameters: (text) => {
      const monthMatch = text.match(MONTHS_REGEX);
  ...
  // Competitor analysis
  {
    ...
    message:
      "I can analyze competitor rates and market positioning! Want me to generate a competitive analysis?",
    confidence: 0.8,
  },
  ...
  // Marketing and landing pages
  {
    ...
    message:
      "I can create landing pages and marketing campaigns! Want me to generate promotional content?",
    confidence: 0.9,
    extractParameters: (text) => {
      const eventMatch = text.match(EVENT_TYPE_REGEX);
      const promoMatch = text.match(PROMO_REGEX);
      return {
    ...
    if (finalScore > highestScore && finalScore > 1) {
      ...
      bestMatch = {
        ...
        parameters: pattern.extractParameters
          ? pattern.extractParameters(text)
          : undefined,
      };
    }
  }
  ...
  if (suggestion) {
    switch (suggestion.actionType) {
      ...
      case "marketing":
        ...
        break;
      default:
        quickActions.push("Execute task", "View details", "Get help");
        break;
    }
  }
