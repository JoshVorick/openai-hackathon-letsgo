...
export const getOccupancyData = tool({
  ...
  inputSchema: z.object({
    startDate: z.string().describe("Start date in YYYY-MM-DD format (must be 2024-01-01 or later)"),
    endDate: z.string().describe("End date in YYYY-MM-DD format (must be 2026-03-12 or earlier)"),
    ...
    asOfDate: z
      .string()
      .optional()
      .describe("Only count bookings made before this date (defaults to today)"),
  }),
  execute: async ({ startDate, endDate, includeYoYComparison = true, asOfDate }) => {
    try {
      ...
      const currentOccupancy = await getOccupancyForDateRange(startDate, endDate, today);

      let comparisonOccupancy = null;
      if (includeYoYComparison) {
        ...
        if (new Date(startLastYearStr) >= dataStart && new Date(endLastYearStr) <= dataEnd) {
          ...
          console.log('⚠️ [getOccupancyData] Historical dates outside data range, skipping YoY comparison');
        }
      }

      // Calculate summary statistics
      const avgOccupancy = currentOccupancy.length > 0
        ? Math.round((currentOccupancy.reduce((sum, day) => sum + day.occupancyRate, 0) / currentOccupancy.length) * 100) / 100
        : 0;

      const avgOccupancyLastYear = comparisonOccupancy && comparisonOccupancy.length > 0
        ? Math.round((comparisonOccupancy.reduce((sum, day) => sum + day.occupancyRate, 0) / comparisonOccupancy.length) * 100) / 100
        : null;

      const change = avgOccupancyLastYear !== null
